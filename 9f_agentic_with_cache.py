...
def proses_dengan_ai(system_prompt: str, user_prompt: str, fallback_response: Dict) -> Dict:
    ...
    if not AVAILABLE_PROVIDERS:
        st.error("No AI provider selected or available. Please add API keys to Streamlit Secrets.")
        # Jangan panggil st.stop(), tapi raise Error agar blok 'finally' di main bisa menangkap
        raise ProviderUnavailableError("No AI provider selected or available.")
    # --- AKHIR PERBAIKAN ---
...
if st.session_state.get(PG + "df_hasil") is None:
    ...
    try:
        ...
        # Ini akan menangkap ProviderUnavailableError jika tidak ada API keys
        st.error(f"🛑 Proses dihentikan: {e}")
        st.warning(f"Menyimpan hasil parsial (jika ada) untuk {len(st.session_state[PG + 'hasil_partial'])} dosen.")
    
    finally:
        # Blok ini SEKARANG akan selalu berjalan, bahkan jika tidak ada API key
        progress_bar.empty()
        ...
        if not df_new_results.empty or not cached_df.empty:
            ...
            # Jika tidak ada hasil DAN tidak ada error (misal, 0 dosen diproses)
            if PG + "df_hasil" not in st.session_state:
        ...
        # Reset 'started' agar tombol tidak nonaktif
        st.session_state[PG + "started"] = False
...
if df_hasil is not None:
    # Jangan tampilkan "Analisis Selesai" jika DataFrame kosong
    ...
    with tab3:
        ...
        if "Confidence Score" not in df_hasil.columns:
            st.warning("Kolom 'Confidence Score' tidak ditemukan.")
             ...
             st.info("Tabel hasil kosong, tidak ada statistik untuk ditampilkan.")
        else:
