...
for k, v in [
    ...
    (PG + "processing", False), # <-- State untuk menonaktifkan UI
]:
...
if st.session_state.get(PG + "df_hasil") is None:
    if not all(files.values()):
        ...
        st.session_state[PG + "processing"] = False # <-- Gagal, set proses ke False
        st.stop()
    ...
    if len(dosen_to_process) == 0:
         ...
         st.session_state[PG + "processing"] = False # <-- Selesai, set proses ke False
         st.rerun()
